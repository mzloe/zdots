# colors.toml parser and semantic color resolver.
#
#   ze_load_colors <colors.toml>   fill THEME_COLORS with every resolved key
#   ze_color <key> [fallback]      print one value; fallback is tried as a key, then used verbatim
#   ze_colors_all                  print every key<TAB>value, sorted
...
parse_colors_file() {
  ...
  while IFS='=' read -r key value; do
    ...
    if [[ ! $key =~ ^[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-]+$ ]]; then
      printf 'ze: skipping key with unsupported characters\n' >&2
      continue
    fi

    if [[ ! $value =~ ^[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789#(),._+/%\ -]*$ ]]; then
      printf 'ze: skipping %s: unsupported characters in value\n' "$key" >&2
      continue
    fi
...
ze_load_colors() {
  ...
  parse_colors_file || return 1
  resolve_theme_colors
}

ze_color() {
  local value="${THEME_COLORS[$1]:-}"

  if [[ -z $value && -n ${2:-} ]]; then
    value="${THEME_COLORS[$2]:-$2}"
  fi
